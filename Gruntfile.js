module.exports = function (grunt) {
    ...
    grunt.initConfig({
            ...
            html: {
                ...
                options: {
                    process: function (content, _srcpath) {
                        ...
                        const templates = grunt.file.expand({ filter: "isFile" }, [`${assetsPath}*`]);
                        templates.forEach(filePath => {
                            const key = filePath.replace(assetsPath, '').replace('.html', '')
                            content = content.replace(`{{${key}}}`, grunt.file.read(filePath));
                        });
                        return content
                    }
                }
        ...
        concurrent: {
            ...
            build: {
                tasks: ["js-deployment", "css-deployment", "html-deployment", "api-deployment", "img-deployment"]
            },
            watch: {
                tasks: ['watch:api', 'watch:js', 'watch:css', 'watch:img', 'watch:html']
            }
        }
    });
